...
async function saveConfig(options) {
    ...
    const filteredOptions = Object.fromEntries(Object.entries(options).filter(([key]) => key !== "save"));
    await fs.writeFile(webextensiontoolboxrcPath, stringify(filteredOptions));
    console.log(`${green(bold("✔"))} Saved ${dim(".webextensiontoolboxrc")} to ${green(bold(webextensiontoolboxrcPath))}`);
    process.exit(0);
}
/**
...
function addSharedOptions(cmd) {
    return cmd
        ...
        .option("--vendor-version [vendorVersion]", "last supported vendor (default: current)")
        .option("--copy-ignore [copyIgnore...]", "Do not copy the files in this list, glob pattern")
        .option("--compile-ignore [compileIgnore...]", "Do not compile the files in this list, glob pattern")
        .option("--no-manifest-validation", "Skip Manifest Validation")
        .option("--save", `Save config to ${dim(".webextensiontoolboxrc")}`)
        .option("--verbose", "print messages at the beginning and end of incremental build");
}
/**
...
export default async function command() {
    ...
    const { version } = JSON.parse((await fs.readFile(`${__dirname}/../package.json`)).toString());
    program
...
`
        ...
        console.log(`${green(bold("✔"))} Loaded ${dim(".webextensiontoolboxrc")} from ${green(bold(webextensiontoolboxrcPath))}`);
    ...
    addSharedOptions(program
        ...
        .option("--no-auto-reload", "Do not inject auto reload scripts into background pages or service workers", true)
        .option("-p, --port [port]", "Define the port for the websocket development server", "35729")
        .option("--dev-server [devServer]", "use webpack dev server to serve bundled files", false)
        .action(async (vendor, options) => {
    ...
    addSharedOptions(program
        ...
        .option("-o,--output-filename [outputFilename]", "Override the output filename")
        .action(async (vendor, options) => {
        await saveConfig(options);
