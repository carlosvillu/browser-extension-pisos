...
export default async function webpackConfig({ src = "app", target = "build/[vendor]", packageTarget = "packages", dev = false, copyIgnore = [], compileIgnore = [], devtool = false, minimize = false, vendor = "chrome", vendorVersion = "", manifestValidation = true, port = 35729, autoReload = false, swc = false, outputFilename, }) {
    if (!browserslistData[vendor]) {
        throw new Error(`${vendor} is not a valid vendor. Valid options are: ${Object.keys(browserslistData).join(", ")}`);
    }
    ...
    const gep = new GlobEntriesPlugin();
    ...
    config.plugins.push(new WebextensionPlugin({
