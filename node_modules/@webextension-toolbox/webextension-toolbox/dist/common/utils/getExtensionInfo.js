...
async function getManifestJSON(src) {
    try {
        return JSON.parse((await fs.readFile(resolve(src, "manifest.json"))).toString());
    }
    catch (error) {
...
async function getPackageJSON(src) {
    try {
        try {
            return JSON.parse((await fs.readFile(resolve(src, "../package.json"))).toString());
        }
        catch (error) {
            return JSON.parse((await fs.readFile(resolve(src, "package.json"))).toString());
        }
...
export default async function getExtensionInfo(src) {
