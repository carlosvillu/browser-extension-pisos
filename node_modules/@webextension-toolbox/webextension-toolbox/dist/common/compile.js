...
async function loadWebpackConfig(options) {
    if (!options.config) {
        const path = await findUp("webextension-toolbox.config.js");
        if (path) {
            console.error(bold(red(`--config option not provided but ${italic(grey(path))} exists.`)));
            console.error(bold(red(`The config option is now required. Provide ${grey("--config")} or delete/rename ${grey(path)}`)));
            process.exit(-1);
        }
        return options;
    }
    ...
    if (!path) {
        console.log(`${red(bold("x"))} No webpack config found at ${green(bold(options.config))}`);
        process.exit(-1);
    }
    console.log(`${green(bold("✔"))} Loading webpack config from ${green(bold(path))}`);
...
export default async (options) => {
    ...
    const userWebpackHook = webpack || ((conf) => conf);
