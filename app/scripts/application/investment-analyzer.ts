import type { PropertyData, UrlAnalysisResult } from '../domain/interfaces';
...
import { type IErrorHandler, RobustErrorHandler } from '../domain/services/error-handler';
import { type IProfitabilityCalculator, ProfitabilityCalculator } from '../domain/services/profitability-calculator';
import { type IPropertyExtractor, PropertyExtractor } from '../domain/services/property-extractor';
import { type IRentalDataAnalyzer, RentalDataAnalyzer } from '../domain/services/rental-data-analyzer';
import { IntersectionSimpleLazyLoader, type SimpleLazyLoader } from '../domain/services/simple-lazy-loader';
import { type IUrlAnalyzer, UrlAnalyzer } from '../domain/services/url-analyzer';
import { CrossReferenceUrlGenerator, type IUrlGenerator } from '../domain/services/url-generator';
import { Logger } from '../infrastructure/logger';
import { type IUIRenderer, InvestmentUIRenderer } from '../presentation/ui-renderer';
...
export class IdealistaInvestmentAnalyzer implements IInvestmentAnalyzer {
  ...
  private async analyzePropertyProfitability(property: PropertyData, urlAnalysis: UrlAnalysisResult): Promise<void> {
    this.logger.log(
      `Analyzing property ${property.id}: ${property.price}€ - ${property.rooms} hab., ${property.size}m²`,
    );
